...
driver.prototype.add = function(opts, lms, mac) {
  var self = this;
  self.app.log.debug('(Squeezebox) : Going to add subs for %s.', mac);

  var parentDevice = new LMSDevice(opts, self._app, lms, mac);
  //console.log('++++++TTTTTTTT ',parentDevice);
...
function LMSDevice(opts, app, lms, mac) {
  ...
  //console.log(JSON.stringify(lms.players[mac]));
